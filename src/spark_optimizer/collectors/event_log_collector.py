...
class EventLogCollector:
    ...
    def parse_event_log(self, log_file: Path) -> Optional[SparkJobMetrics]:
        ...
        end_time = datetime.fromtimestamp(app_end["Timestamp"] / 1000) if app_end else None
        ...
        for event in events:
            if event.get("Event") == "SparkListenerTaskEnd":
                ...
                if task_metrics:
                    ...
                    metrics["output_bytes"] += task_metrics.get("Output Metrics", {}).get("Bytes Written", 0)
                    metrics["shuffle_read_bytes"] += task_metrics.get("Shuffle Read Metrics", {}).get("Total Bytes Read", 0)
                    metrics["shuffle_write_bytes"] += task_metrics.get("Shuffle Write Metrics", {}).get("Bytes Written", 0)
                    ...
                    metrics["peak_memory"] = max(metrics["peak_memory"], task_metrics.get("Peak Execution Memory", 0))
                    
                    metrics["executor_run_time"] += task_metrics.get("Executor Run Time", 0)
                    metrics["executor_cpu_time"] += task_metrics.get("Executor CPU Time", 0)
