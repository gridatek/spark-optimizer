...
def get_recommendation():
    ...
    try:
        ...
        if 'input_size_bytes' not in data:
            return jsonify({
                'error': 'input_size_bytes is required'
            }), 400
        ...
        # Get recommendation
        logger.info(f"Generating recommendation for {input_size} bytes, priority={priority}")
...
def list_jobs():
    ...
    try:
        ...
        try:
            ...
            if min_duration:
                query = query.filter(SparkJob.duration_ms >= min_duration * 1000)
            
            if max_duration:
                query = query.filter(SparkJob.duration_ms <= max_duration * 1000)
...
def analyze_job(app_id: str):
    ...
    try:
        ...
        try:
            ...
            if issues:
                efficiency_score -= len([i for i in issues if i['severity'] == 'high']) * 20
                efficiency_score -= len([i for i in issues if i['severity'] == 'medium']) * 10
            ...
            return jsonify({
                ...
                'summary': f"Found {len(issues)} potential issues" if issues else "No major issues detected"
            })
...
def get_stats():
    ...
    try:
        ...
        try:
            ...
            avg_duration = session.query(func.avg(SparkJob.duration_ms)).scalar()
            total_input = session.query(func.sum(SparkJob.input_bytes)).scalar()
